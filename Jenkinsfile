pipeline {
    ...
    environment {
        ...
        RELEASE = "1.0.0"
        DOCKER_USER = "omar421"
        DOCKER_PASS = credentials('DOCKER_PASS') // Use credentials for Docker password
    ...
    stages {
        ...
        stage("Trigger CD Pipeline") {
            steps {
                script {
                    sh "curl -v -k --user clouduser:${JENKINS_API_TOKEN} -X POST -H 'cache-control: no-cache' -H 'content-type: application/x-www-form-urlencoded' --data 'IMAGE_TAG=${IMAGE_TAG}' 'http://ec2-3-221-170-75.compute-1.amazonaws.com:8080/job/gitops-register-app-cd/buildWithParameters?token=gitops-token'"
                }
            }
        }
    }
